...
BEGIN_MESSAGE_MAP(CFilterVCMPage, CMFCPropertyPage)
	ON_WM_SIZE()
...
CFilterVCMPage::CFilterVCMPage(LPUNKNOWN pUnk, HRESULT *phr, LPCTSTR strTitle) :
	CMFCPropertyPage(_T("FilterVCMDialogs"), pUnk, IDD, strTitle)
{
	// retval
	if (phr) *phr = NOERROR;
	vfwdialogs = NULL;
...
BEGIN_MESSAGE_MAP(CVideoCompressionPage, CMFCPropertyPage)
	ON_WM_HSCROLL()
...
CVideoCompressionPage::CVideoCompressionPage(LPUNKNOWN pUnk, HRESULT *phr, LPCTSTR strTitle) :
	CMFCPropertyPage(_T("VCMPage"), pUnk, IDD, strTitle)
{
	// retval
	if (phr) *phr = NOERROR;
	comp = NULL;
...
BOOL CVideoCompressionPage::OnInitDialog()
{
	...
	bool		kf = flags & CompressionCaps_CanKeyFrame;
	bool		bf = flags & CompressionCaps_CanBFrame;
	bool		cq = flags & CompressionCaps_CanQuality;
	bool		cw = flags & CompressionCaps_CanWindow;
